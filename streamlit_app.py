...
if st.button("Procesar imagen", type="primary", use_container_width=True):
    ...
    try:
        ...
        log.write("5/6 Preparando PNG...")
        st.session_state["result_img"] = work
        st.session_state["result_png"] = image_to_png_bytes(work, dpi=int(dpi))
        progress.progress(85)

        log.write("6/6 Preparando PDF...")
        st.session_state["result_pdf"] = image_to_pdf_bytes(
            work,
            dpi=int(dpi),
        ...
        if make_ht:
            ...
            st.session_state["halftone_pdf"] = image_to_pdf_bytes(
                ht,
                dpi=int(dpi),
                white_background=True,
            )
...
if "result_img" in st.session_state:
    ...
    bg_mode = st.radio(
        "Vista previa",
        ["Transparente", "Negro", "Blanco", "Gris"],
        horizontal=True,
    )
    ...
    with colB:
        ...
        st.download_button(
            ...
            data=st.session_state["result_png"],
            file_name="mc_dtf_pro_limpio.png",
            mime="image/png",
        ...
        st.download_button(
            ...
            file_name="mc_dtf_pro_limpio.pdf",
    ...
    if "halftone_img" in st.session_state:
        st.subheader("Semitono")
        st.image(
            composite_preview(st.session_state["halftone_img"], bg_mode),
            use_container_width=True,
        )

        st.download_button(
            "Descargar PNG semitono",
            data=st.session_state["halftone_png"],
            file_name="mc_dtf_pro_semitono.png",
            mime="image/png",
        )

        st.download_button(
            "Descargar PDF semitono",
            data=st.session_state["halftone_pdf"],
            file_name="mc_dtf_pro_semitono.pdf",
            mime="application/pdf",
        )
