...
def render_internal_residue_stats(debug: dict[str, object]) -> None:
    ...
    st.metric("Componentes protegidos", str(stats.get("internal_components_protected", 0)))
    st.metric("Componentes en revision", str(stats.get("internal_components_review", 0)))
    st.metric("Area detectada", str(stats.get("internal_residue_area", 0)))
    st.metric("Area eliminada", str(stats.get("internal_removed_area", 0)))
    st.metric("Pixeles eliminados", str(stats.get("internal_removed_pixels", 0)))
    st.metric("Cambio alpha", f"{stats.get('internal_alpha_change_percent', 0)}%")
    st.metric("Alta confianza", str(stats.get("internal_high_confidence", 0)))
    st.caption(f"IDs eliminados: {stats.get('internal_removed_ids', [])}")
    st.caption(f"IDs protegidos: {stats.get('internal_protected_ids', [])}")
    st.caption(f"IDs en revision: {stats.get('internal_review_ids', [])}")
    st.caption(f"IDs manuales encontrados: {stats.get('internal_manual_found_ids', [])}")
    st.caption(f"IDs manuales rechazados: {stats.get('internal_manual_rejected_ids', [])}")
    reasons = stats.get("internal_protection_reasons", {})
    if reasons:
